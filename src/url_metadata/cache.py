...
from .model import Metadata, LassieMetadata
...
class URLMetadataCache:
    def __init__(
        ...
        if not cdir.exists():
            cdir.mkdir()
        self._base_cache_dir = cdir

        self.cache_dir = self._base_cache_dir / "data"
        if not self.cache_dir.exists():
            self.cache_dir.mkdir()

        self.database_path = self._base_cache_dir / "index.sqlite"

        self.subtitle_language = subtitle_language
        self.logger = setup(loglevel)
        self.sleep_time = sleep_time

        self.lassie = Lassie()
        # hackery with a requests.Session to save the most recent request object
        self.lassie.client = SaveSession(cb_func=self.save_http_response)
    ...
    def handle_url(self, url: str) -> Metadata:
        ...
        metadata.info: LassieMetadata = self.lassie.fetch(url, handle_file_content=True)
        sleep(self.sleep_time)
